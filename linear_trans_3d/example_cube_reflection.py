...
from transformations import reflection_3d, projection_matriz
from cube_example import cube

u = np.sqrt(3) * np.array([1,1,1]).reshape(-1, 3)
...
cubes_reflected = [rcube.reflectedByMatrix(H) for rcube in cubes_rotatedx]
...
def update(i):
    ...
    cubes_reflected[i].showPolyedrom(color='red')
    cubes_projected[i].showPolyedrom(color='gray')
    return line,

ani = animation.FuncAnimation(fig, update, frames=100, interval=100)
ani.save('reflection_cube.gif')